...
def discretisationMatrix(N):
    """
    Sparse version of your discretisationMatrix.
    Returns:
      A: scipy.sparse.csr_matrix, shape ((N+1)^2,(N+1)^2), complex
      b: np.ndarray, shape ((N+1)^2,), complex
    Uses your global h and your f,g exactly like before.
    """
    import numpy as np
    ...
    # Helper: your exact global indexing
    def idx(i, j):
...
def gauss_seidel_v2(A, b, Nx, Ny, x0=None, tol=1e-10, max_iter=500, verbose=True):
    ...
    # --- keep same structure, but DO NOT cast to float (would destroy complex + densify sparse) ---
    if issparse(A):
        A = A.tocsr()
    else:
    ...
    # Precompute for sparse row access
    if issparse(A):
        indptr = A.indptr
        indices = A.indices
        data = A.data
    ...
    for k in range(1, max_iter + 1):
        ...
        if issparse(A):
            ...
            # --- dense Gauss–Seidel (your original logic) ---
            for j in range(Ny):
                for i in range(Nx):
                    p = j * Nx + i
        ...
        # check convergence (keep your output style)
        diff = np.linalg.norm(x - x_old, ord=np.inf)
